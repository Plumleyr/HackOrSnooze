...
class StoryList {
  ...
  async editStory(user, storyId, {author, title, url}){
    const token = user.loginToken;
    const res = await axios({
      url: `${BASE_URL}/stories/${storyId}`,
      method: "PATCH",
...
class User {
  ...
  async updateName(value){
    $(".user-name").text(`Name: ${value}`);
    await this.updateUser("name", value);
  }

  async updatePassword(value){
    await this.updateUser("password", value);
  }
